...
function queueMediaItems(mediaInfoItems) {
  requestSession().then()
    .then((session) => {
      const queueItems = mediaInfoItems.map((mediaInfo) => new chrome.cast.media.QueueItem(mediaInfo));
      const queueRequest = new chrome.cast.media.QueueLoadRequest(queueItems.slice(0, 35));
      session.queueLoad(queueRequest, setupCurrentMedia, onCastError);
    })
...
function pause() {
  getCurrentMedia() && getCurrentMedia().play(new chrome.cast.media.PauseRequest());
}
